...
local function isDifferent(A, b)
    ...
    local t = typeof(A)
    return ((t == "Color3" and (A.R ~= b.R or A.G ~= b.G or A.B ~= b.B)) or (t == "UDim2" and (A.X.Scale ~= b.X.Scale or A.X.Offset ~= b.X.Offset or A.Y.Scale ~= b.Y.Scale or A.Y.Offset ~= b.Y.Offset)) or ((t == "Font" or t == "FontFace") and tostring(A) ~= tostring(b)) or (t == "Vector3" and (A.X ~= b.X or A.Y ~= b.Y or A.Z ~= b.Z)) or (t == "CFrame" and (A.Position ~= b.Position or A.XVector ~= b.XVector or A.YVector ~= b.YVector or A.ZVector ~= b.ZVector)) or (A ~= b))
end

local function CheckDiff(obj, Prop, NewValue)
    local OldValue = obj[Prop]
    local safeName = obj.Name or "Unnamed"
    if isDifferent(OldValue, NewValue) then
        obj[Prop] = NewValue
    end
end
